 ...
 * @returns {Promise}
 */
function createAmiConnection(login, secret, connectionOptions, tls) {
  return new Promise((resolve, reject) => {
    ...
    const net = tls ? require('tls') : require('net');
  ...
  return function (login, password, options) {
    return co(function* () {
      ...
      while (
        ...
        ++currAttemptIndex <= maxAttemptsCount
      ) {
        try {
        ...
        if (maxAttemptsCount === null) {
          ...
          debugLog(
            `attempt [${currAttemptIndex} of ${maxAttemptsCount}] of reconnecting...`
          );
        }
      }
...
module.exports = function (options) {
  ...
  return {
    connect: !options.reconnect
      ? createAmiConnection
      : wrapper(options.maxAttemptsCount, options.attemptsDelay)
  };
};
